...
class StudentModel{
    ...
    public function getStudentDetails($user_ID) {
        $this->db->query('SELECT s.studentID, s.fName, s.lName, s.gender, s.dateOfBirth, s.nicNumber, s.institutionName, s.institutionNameVisibility, s.studentType, s.caregiverName, s.caregiverType, s.caregiverRelationship, s.caregiverOccupation, s.studyingYear, d.phoneNumber, d.branchName, d.bankName, d.accNumber, d.accountHoldersName, d.address, s.receivingScholarships FROM student s JOIN donee d ON s.studentID = d.doneeID WHERE studentID = :studentID;');
